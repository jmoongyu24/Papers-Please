...
| arXiv 검색어 생성 | `Qwen3-4B-Instruct-2507` + LoRA 를 합쳐 4비트로 | Ollama `papers-rewriter` | 3.2GB |
...
언어 모델 하나가 3.25GB 라 재정렬 모델과 같은 시각에 올라가면 4.76GB 가 되기 때문이다.
...
변환기를 4비트로 줄이면 검색 최대치가 3.3GB 로 내려간다. 정밀도를 올린 판도 만들어
...
f16 기준으로 q8_0 은 세 지표 모두 판정 불가이고(차이 0.006 이하), q4_K_M 은
MRR(p=0.034)과 nDCG(p=0.002)가 유의미하게 낮다. Recall 은 셋 다 구분되지 않는다.

**q8_0 이 품질 대비 이득이 크지만 4비트를 쓴다.** 4GB 목표를 지키려는 것이고,
...
```bash
# arXiv 검색어 변환기를 Ollama 에 등록 (llama.cpp 변환 스크립트 필요)
...
검색하고 화면에 그렇게 알린다. `--grades` 를 빼면 q8_0 과 f16 판까지 만든다 (성능을
