...
네 단계다. 2단계와 3단계는 무료, 1단계와 4단계는 OpenAI 를 쓰므로 돈이 든다.
...
# (4) 후보마다 관련도 0~3 판정 (유료, --max-cost 로 한도를 걸 수 있다)
...
```bash
# 서비스와 같은 구성으로 개발용 전체 (중단하면 이어서 함)
...
성능을 재는 것이 아니라 고장을 잡는 것이다. 떨어지면 Recall 을 볼 필요가 없다.
...
4. **주 지표를 대신하지 않는다.** 채택 여부는 Recall@10 으로 정하고, nDCG 는
...
| 항목 | 값 | 근거 |
|---|---|---|
| 후보 깊이 | 100 | 300 이면 만족도가 떨어짐 (p<0.001) |
