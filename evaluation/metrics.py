...
def bootstrap_ci(scores: List[float], n_boot: int = 10000, seed: int = 42,
    ...
    질문 목록에서 중복을 허용해 같은 개수만큼 다시 뽑기를 n_boot 번 반복하고, 그때마다의 평균이 어느 범위에 흩어지는지 봄
...
def paired_bootstrap(
    ...
    질문 목록에서 중복을 허용해 다시 뽑기를 n_boot번 반복해 '후 - 전' 평균 차이의 분포를 봄
...
def ndcg_at_k_single(ranked: List[str], gold_map: Dict[str, int], k: int) -> float:
    ...
    등급을 2^등급-1로 바꿔 이득으로 쓰고(높은 등급을 더 크게 보상), 이상적 정렬 대비
    비율로 정규화하므로 0~1 사이의 값을 가짐
    """
