...
중간 산출물(질문 원본, 후보 풀)은 **`runs/` 에 둔다.** `data/eval/` 에는 실제로 평가에 쓰는
네 개만 남긴다 — `dev.jsonl`, `test.jsonl`, `grades_dev.jsonl`, `grades_test.jsonl`.
중간 파일이 평가 폴더에 쌓이면 "무엇이 진짜 시험지인지" 알 수 없게 된다.

## 이 평가셋이 고친 것 — 옛 평가셋의 설계 결함 세 가지

**① 난이도 이름이 설계 의도와 뒤집혀 있었다.**
옛 코드의 `hard` 는 **대학원생의 정확한 질문**이고 `easy` 는 **초심자의 일상어 질문**이었다.
원래 의도(easy=쉬운 질문)와 정반대라 "어려울수록 잘 맞힌다"는 이상한 표가 나왔다(ISSUE #32).

**② 제목을 모델에게 주고 "어순만 바꿔라"라고 시켰다 — 누수의 주범이다.**
실측(2026-08-13): 대학원생 층 영어 질문은 제목 내용어의 **중앙값 57%** 를 담고 있었고
Recall@10 이 **1.000** 이었다. 하나도 안 틀렸다. 결정적인 것은 그 층에서 **'이 논문만의 고유
용어'를 걷어내도 여전히 1.000** 이라는 점이다. 원인은 희귀 용어가 아니라 **제목의 낱말 조합을
그대로 재현한 것**이다. `image captioning`(71만 편 중 1,136편) 처럼 흔한 말만 써도, 조합이
제목과 같으면 검색기에게는 정답을 적어 준 것과 같다.

    → 이제 **제목을 아예 주지 않는다.** 초록만 주고, "이 논문을 아직 못 찾은 사람"의
      자리에서 쓰게 한다. 이것이 가장 효과가 큰 한 가지 장치다.

**③ 한국어 문항은 누수 검사를 받은 적이 없다.**
옛 검사기가 영어 낱말만 세기 때문에, 한국어 질문의 낱말 집합은 영어 논문 용어와 **항상
공집합**이었다. "깨끗하다"가 아니라 "검사한 적이 없다"가 맞다. 실제로 제목을 통째로 번역한
문항이 통과해 있었다.

    → 이제 **한 번의 호출로 한국어와 영어를 함께** 만든다. 그러면 한국어 문항에도 영어판이
      항상 있으므로 모든 누수 검사가 영어 위에서 돌아간다. 비용은 오히려 절반이다.
...
지금 축은 검색 방식과 무관하다 — 묻는 사람의 전문성이 기준이다.
...
다르기 때문이다. 지금은 같은 논문·같은 난이도를 두 언어로 만들어 다른 조건을 모두 고정한다.
`pair_id` 가 그 짝을 잇는 열쇠다.
"""
...
def content_words(s: str) -> set[str]:
    """겹침을 잴 때 세는 낱말만 남긴다 (흔한 말과 2글자 이하는 뺀다).

    **영어 낱말만 센다.** 한국어 질문에 그대로 쓰면 항상 공집합이 나와 "깨끗하다"고
    잘못 판정한다(ISSUE #32). 한국어는 반드시 `audit` 이 영어로 옮긴 뒤에 넣을 것.
    """
...
# 난이도 이름만 봐서는 누가 물었는지 알 수 없으므로 표에 함께 적는다
WHO = {"easy": "대학원생·정확한 학술어", "medium": "학부연구생·섞인 표현",
       "hard": "1~2학년·일상어", "known_item": "논문을 아는 사람"}


# ══════════════════════════════════════════════════════════════════════════
# 1단계. generate — 논문에서 질문을 거꾸로 만든다 (유료, OpenAI)
# ══════════════════════════════════════════════════════════════════════════
...
def sample_papers(corpus: str, n: int, exclude: set[str], seed: int,
    ...
    """코퍼스에서 논문을 뽑는다. **이미 평가에 쓰인 정답 논문은 제외한다.**
...
# ══════════════════════════════════════════════════════════════════════════
# 2단계. split — 개발용(dev)과 시험용(test)으로 나눈다 (무료)
# ══════════════════════════════════════════════════════════════════════════
#
# ## 왜 논문 단위로 나누는가
#
# 질문이 아니라 **정답 논문**을 기준으로 나눈다. 같은 논문에서 나온 질문이 양쪽에 흩어지면,
# 개발하면서 시험 논문을 미리 보는 셈이 되어 성적이 부풀려진다.
#
...
#     대신 분포를 그대로 남겨 두고 **성능을 겹침 구간별로 층화 보고**한다(`audit`).
...
def cmd_split(args) -> None:
    ...
    for r in kept:
        by_paper[r["gold_id"]].append(r)
    # 난이도 × 언어 문항이 온전한 논문만 쓴다 — 층 균형이 저절로 맞는다
...
# ══════════════════════════════════════════════════════════════════════════
# 3단계. pool — 등급을 매길 후보 풀을 만든다 (로컬 검색만, 비용 0원)
# ══════════════════════════════════════════════════════════════════════════
#
# ## 왜 풀이 필요한가
#
# 단일 정답 지표는 "질문 하나에 정답 논문 딱 1편"을 센다. 그런데 실제로는 한 질문에 맞는
# 논문이 여러 편 있어서, 시스템이 다른 좋은 논문을 1등에 올려도 **0점**을 받는다. 이것이
# 이론적 상한(ISSUE #14)의 정체다. 그 한계를 풀려면 **후보마다 관련도를 매긴 등급
# 정답지**가 필요하고, 그러려면 먼저 판정할 후보를 모아야 한다.
#
# ## 한 짝(한국어·영어)당 한 번만 매긴다
#
# 두 문항은 **같은 정보 요구**를 다른 언어로 표현한 것이므로, "이 논문이 그 요구를
# 만족시키는가"의 답은 같다. 관련도는 언어가 아니라 뜻의 문제이기 때문이다.
#     → 판정은 **영어판으로 한 번만** 하고 두 문항이 그 결과를 함께 쓴다. 비용이 절반이 된다.
#
...
# 것은 편향이 아니다 — 실제 서비스도 그 논문을 결과로 내놓는다. ISSUE #22 에서 문제였던 것은
...
# ══════════════════════════════════════════════════════════════════════════
# 4단계. grade — 후보마다 관련도를 0~3 으로 매긴다 (유료, OpenAI)
# ══════════════════════════════════════════════════════════════════════════
#
# ## 얻는 것 세 가지
#
#   1. nDCG — 지정 정답이 아니어도 좋은 논문을 위에 올리면 점수를 준다
#   2. 이 시험지의 진짜 이론적 상한을 새로 계산할 수 있다
#   3. 시스템이 못 찾은 것인지, 잘 찾았는데 지표가 못 알아본 것인지 구분된다
#
# ## 심판을 생성기와 다른 계열로 쓴다 (ISSUE #34)
#
# 옛 등급 정답지 90문항은 질문을 만든 모델과 채점한 모델이 둘 다 gpt-4o-mini 였다.
# 자기가 만든 질문에 후하게 주면 만족 논문 수가 부풀려지고, 그만큼 이론적 상한이
# 과소평가된다. 지금은 생성기 gpt-5.4, 심판 gpt-4.1-mini 로 계열을 갈랐다.
# gpt-4.1-mini 는 비추론 모델이라 출력 토큰이 예측 가능하다 — 추론 토큰이 숨어서 붙으면
# 예산이 몇 배로 튄다.

# gpt-4.1-mini 단가 (1M 토큰당 달러). 다른 모델을 쓰면 이 값을 바꿔야 비용 추정이 맞다.
...
def cmd_grade(args) -> None:
    ...
    for i, row in enumerate(pool, 1):
        ...
        # 판정은 영어판으로만 한다 (관련도는 언어가 아니라 뜻의 문제)
...
# ══════════════════════════════════════════════════════════════════════════
# audit — 만든 평가셋에 정답이 새어 있는지 검사한다 (로컬 번역, 비용 0원)
# ══════════════════════════════════════════════════════════════════════════
#
# ## 한국어 문항을 어떻게 검사하는가
#
...
def ollama_json(model: str, system: str, user: str, timeout: int = 120) -> dict:
    ...
    `ollama` 파이썬 패키지를 안 쓰는 이유: 감사 도구는 의존성이 적을수록 좋다. 이 기능은
    "지금 이 평가셋을 믿어도 되는가"를 판정하는 도구라서, 나중에 다른 사람이 다른 환경에서
    그대로 돌려 볼 수 있어야 한다. 표준 라이브러리만 쓰면 그게 보장된다.
    (실제로 이 판단이 값을 했다 — 의존성 9개가 사라졌던 ISSUE #35 때 이 도구만 살아 있었다.)
    """
...
def _gold_papers(rows: list[dict], corpus: str | None) -> dict[str, dict]:
    ...
    평가셋 문항이 `_title` 을 갖고 있으면 그것을 쓴다(코퍼스 1GB 를 훑지 않아도 된다).
    없으면 코퍼스에서 찾는다.
    """
