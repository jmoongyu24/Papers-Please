"""평가 결과 읽고 보고하기 — 실행 하나를 자세히 보거나, 여러 실행을 짝지어 비교한다.
...
`--run` 을 하나 주면 자세한 보고, 둘 이상 주면 비교표가 나온다.


## 왜 두 지표를 함께 보는가

**단일 정답 지표(Recall@10)** 는 "질문을 만들 때 쓴 그 논문 1편"을 찾았는지만 센다.
그런데 등급 정답지를 실제로 재보니 **질문 하나에 정답급(3등급) 논문이 중앙값 2편**이고,
절반 이상의 질문에서 2편 이상이었다. 즉 시스템이 진짜 좋은 논문을 1등에 올려도 그게
지정된 출처가 아니면 **0점**을 받는다.

**만족도 지표(nDCG@10)** 는 등급을 이득으로 써서 "좋은 논문을 위에 올렸는가"를 잰다.
사용자가 실제로 느끼는 품질에 가깝다.

두 숫자가 크게 갈리면 그 차이가 곧 **지표의 한계이지 시스템의 실패가 아니다.**
그래서 하나만 보고 판단하지 않는다.

## 비교할 때 공통 문항만 쓰는 이유 (ISSUE #12)

모델마다 실패한 문항이 다르다. arXiv 오류로 A 는 38문항, B 는 40문항이 평가됐다면 두 평균을
그냥 비교하는 것은 **서로 다른 시험지를 비교하는 것**이다. 실제로 이 프로젝트에서 arXiv
HTTPError 7건 때문에 비교가 왜곡된 적이 있다. 그래서 모든 결과에서 **공통으로 나온 문항만**
추려 비교하고, 같은 질문끼리 짝지어 부트스트랩 검정을 돌린다.

평균 차이만으로는 우연인지 알 수 없다 — 40문항에서 +0.05 는 잡음일 수 있고 300문항에서
+0.05 는 실제일 수 있다. 실제로 "로컬 단독 0.677 vs 두 채널 0.680" 의 차이는 p=0.889 로
순수한 잡음이었는데, 그 잡음을 보고 설정을 골랐다가 시험지를 태웠다(ISSUE #25).

## 언어 짝을 이용한 비교

평가셋은 같은 논문·같은 난이도를 한국어와 영어로 만들어 `pair_id` 로 이어 두었다.
그래서 언어별 성능 차이를 **논문 차이와 뒤섞이지 않게** 볼 수 있다. 옛 평가셋은 한국어
질문과 영어 질문이 서로 다른 논문에 대한 것이라 이 비교가 불가능했다.
"""
...
def load_run(path: str) -> tuple[str, dict[str, list[str]]]:
    """실행 결과를 {문항id: 최종 순위} 로 읽는다.

    재정렬 결과가 있으면 그것을, 없으면 융합 결과를, 그것도 없으면 첫 채널을 쓴다.
    이름은 메타 줄의 변환기 이름을 쓰고, 없으면 파일 이름을 쓴다.
    """
...
def score_rows(run: dict[str, list[str]], queries: list[dict],
    ...
    for q in queries:
        ...
        # 등급 정답지는 짝(pair) 단위다. 관련도는 언어가 아니라 뜻의 문제라
        # 한국어와 영어 문항이 같은 채점표를 함께 쓴다.
...
def report_single(name: str, rows: list[dict], k: int) -> None:
    ...
    # 제목 겹침 구간별 — 누수가 성능을 얼마나 떠받치는지 층으로 본다
...
def report_compare(named_rows: list[tuple[str, list[dict]]], k: int) -> None:
    """공통 문항만 추려 비교하고, 첫 번째를 기준으로 짝지은 검정을 돌린다."""
    ...
    print(f"\n■ '{base_name}' 대비 차이 (같은 문항끼리 짝지어 부트스트랩 검정)")
    print(f"\n{'실행':<26}{'차이':>9}{'95% 신뢰구간':>22}{'p':>9}   판정")
    for name, s in list(scored.items())[1:]:
        st = paired_bootstrap(scored[base_name]["hit"], s["hit"])
    ...
    print("\n※ p 가 0.05 보다 크면 '차이가 없다'가 아니라 **'있는지 없는지 모른다'** 이다.")
    print("   모르는 차이를 근거로 설정을 고르면 시험지만 소모된다(ISSUE #25).")
