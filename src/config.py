...
다른 모듈은 여기서 가져다 씀. 값을 바꿀 일이 생기면 이 파일만 고치면 됨.
"""
...
def _fix_libstdcxx() -> None:
    """torch 보다 먼저 새 C++ 표준 라이브러리를 올려 둠.

    무엇을 고치는가:
    torch 를 먼저 부르면 시스템의 `/lib/x86_64-linux-gnu/libstdc++.so.6` 가 먼저 잡힘.
    그런데 그 파일에는 `CXXABI_1.3.15` 가 없어서, 뒤이어 pyarrow(sklearn 이 부름)를
    올릴 때 아래 오류로 깨짐.
    ...
    왜 이 방식인가:
    `LD_PRELOAD` 환경변수로도 되지만, 그건 실행할 때마다 사람이 붙여 줘야 함. 잊으면
    어떤 경로에서는 되고 어떤 경로에서는 안 되는 상태가 됨. 여기서 미리 올려 두면
    config 를 가져오는 모든 실행 경로에 자동으로 적용됨.

    조건: torch 가 올라오기 전에 실행돼야 함. config 는 거의 모든 모듈이 맨 위에서
    가져오므로 이 자리가 맞음. 라이브러리를 못 찾으면 조용히 넘어감(원래 환경이 멀쩡한
    경우까지 깨뜨리지 않기 위함).
    """
...
# -- 경로 ----------------------------------------------------------------
# 이 파일 기준으로 저장소 최상위 폴더를 계산함 (src/config.py -> 상위의 상위).
...
# data/API_KEY.env 는 "KEY=VALUE" 형식이 아니라 키 값 한 줄만 담긴 파일도 허용함.
# 이 파일은 .gitignore 에 등록되어 있어 절대 커밋되지 않음.
...
def _load_openai_key() -> str:
    ...
    if _API_KEY_FILE.exists():
        ...
        # "OPENAI_API_KEY=sk-..." 형식이면 값만 뽑고, 아니면 파일 전체를 키로 봄.
