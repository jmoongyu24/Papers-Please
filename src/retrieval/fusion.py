"""여러 검색 채널의 결과를 하나로 합친다 — 순위 기반 합치기(RRF).

## 왜 '점수 더하기'가 아니라 '순위 더하기'인가

이 시스템은 성격이 다른 검색 경로를 나란히 둔다(ISSUE #21의 결론).
...
## 반드시 처리해야 하는 함정 — 논문 번호 표기

arXiv 실시간 결과는 `2103.00020v2` 처럼 **버전 표기가 붙어** 오고, 로컬 색인은
`2103.00020` 처럼 붙지 않은 형태로 저장돼 있다. 그대로 합치면 **같은 논문이 서로 다른
논문으로 취급돼 두 채널이 합의한 논문일수록 오히려 점수가 반으로 쪼개진다.** 오류가 아니라
조용히 성능만 깎는 종류의 버그라, 여기서 번호를 한 형태로 통일한 뒤 합친다.

버전 표기를 뗄 때 `paper_id.split("v")[0]` 을 쓰면 안 된다. 옛 형식 번호 중에는
`solv-int/9611001v1` 처럼 **번호 자체에 v가 들어간 것**이 있어 `sol` 로 잘려 버린다.
끝에 붙은 `v숫자` 만 정규식으로 떼어낸다.
"""
...
def normalize_paper_id(paper_id: str) -> str:
    """논문 번호를 채널 사이에서 비교 가능한 한 형태로 만든다.
