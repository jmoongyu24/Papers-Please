...
섞일 여지가 없고, 어차피 변환기를 만드는 모든 경로가 이것을 거친다. 파일을 나눠 두면
"변환기 하나 고치려는데 파일 세 개를 연다"가 된다.

모델을 바꾸거나(qwen3:4b → 1.7b), 서빙 방식을 바꾸거나, 재시도·지연시간 기록 같은 공통 처리를
넣을 때는 `OllamaClient` 만 고치면 된다.
...
class PassthroughRewriter:
    ...
    이 프로젝트의 핵심 주장은 "질문을 변환하면 검색이 좋아진다"이다. 그 주장을 검증하려면
    **변환하지 않았을 때의 값**이 반드시 있어야 하므로, 모든 평가에 이것을 포함한다.
    """
...
def build_rewriter(name: str) -> Rewriter:
    ...
    그 이름을 부를 때만 늦게(lazy) 가져온다. passthrough 는 의존성이 없다.
    """
    if name == "passthrough":
...
class OllamaClient:
    ...
    - generate_json: JSON 스키마를 강제해서, 모델이 스키마를 벗어난 답을 아예 못 내게 한다.
      ("특정 줄을 정규식으로 뽑기"는 형식이 조금만 어긋나도 깨지는데, 그 문제를 원천 차단한다.)
