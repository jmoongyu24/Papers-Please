"""언어 모델에게 시켜서 변환하는 변환기 세 개 — 지금은 모두 **비교 대상(기준선)** 이다.

## 왜 이 셋이 한 파일에 있는가

셋 다 "Ollama 로 Qwen3-4B 를 부르고, JSON 스키마로 출력을 가둔다"는 같은 뼈대를 쓴다.
...
**지금 서비스가 쓰는 변환기는 여기 없다.** 학습한 모델(`finetuned.py` 의 dpo)이 쓰인다.
`HierarchicalRewriter` 는 그 모델의 **학습 데이터를 만드는 데도** 쓰인다
(`training/build_training_data.py`). 그래서 지우지 않고 남겨 둔다.

## 프롬프트를 고칠 때

프롬프트는 가장 자주 고치는 부분이다. 고치면 아래 `PROMPT_VERSION` 을 올려, 어느 프롬프트로
잰 결과인지 나중에 구분할 수 있게 한다.
"""
...
# (대화 기록으로 넣으면 특정 입력이 예시 주제로 붕괴해 그대로 복사되는 문제가 있었다 — ISSUE #1)
...
# ══════════════════════════════════════════════════════════════════════════
# 2. arXiv 검색어 만들기 (코드로 — 모델에게 맡기지 않는다)
# ══════════════════════════════════════════════════════════════════════════
...
class HierarchicalRewriter:
    ...
    한 번에 생성하게 한다. JSON 스키마를 강제하므로 출력이 항상 파싱 가능하다.
